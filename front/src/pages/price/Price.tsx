...
export default function CotizacionPage() {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      const promises = cotizacion.map(async (cotiItem) => {
        return {
          savedCoti: cotiItem,
          component: (await repo.details(cotiItem.category, cotiItem.id))
            .results[0],
        } as CotiItem;
      });
  ...
  return (
    <div className="p-8 pt-20 max-w-screen-xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Cotización</h1>
        {components.length > 0 && (
          <div className="text-xl font-semibold bg-[var(--secondary-background)] border-2 border-border/40 px-4 py-2 rounded shadow-[2px_2px_0px_#000]">
          ...
          {components.map((c) => {
            ...
            return (
              <div
                ...
                className="flex items-center gap-4 border-4 border-border/40 bg-[var(--secondary-background)] p-4 rounded-lg"
              >
                <img
                  src={prod?.img ?? ""}
