...
async function componentsRoutes(
  ...
  fastify.get("/search", async (request, reply) => {
    ...
    if (!category || !query) {
      reply.send({
        status: "bruh",
        text: "Especifica una categoria y una query",
      });
    }
  ...
  fastify.post("/addgpu", async (request, reply) => {
    ...
    if (!name || !memory || !wattage) {
      return reply
        .code(400)
        .send({ status: "error", text: "Faltan parámetros" });
    }
    ...
    try {
      ...
      return reply
        .code(500)
        .send({ status: "error", text: "Error al guardar GPU" });
    }
  });
  ...
  fastify.post("/addcpu", async (request, reply) => {
    ...
    try {
      ...
      return reply
        .code(500)
        .send({ status: "error", text: "Error al guardar CPU" });
    }
  });
  ...
  fastify.post("/addram", async (request, reply) => {
    ...
    try {
      const component = new RAMComponent(
        name,
        brand,
        capacity,
        gen,
      ...
      return reply
        .code(500)
        .send({ status: "error", text: "Error al guardar RAM" });
    }
  });
  ...
  fastify.post("/addpsu", async (request, reply) => {
    ...
    try {
      const component = new PSUComponent(
        name,
        manufacturer,
        model,
        efficiencyRating,
      ...
      return reply
        .code(500)
        .send({ status: "error", text: "Error al guardar PSU" });
    }
  });
  ...
  fastify.post("/addmotherboard", async (request, reply) => {
    ...
    try {
      const component = new MotherboardComponent(
        name,
        Number(year),
        socket,
        chipset,
      ...
      return reply
        .code(500)
        .send({ status: "error", text: "Error al guardar Motherboard" });
    }
  });
  ...
  fastify.post("/addssd", async (request, reply) => {
    ...
    try {
      const component = new SSDComponent(
        name,
        capacity,
        format,
        iface,
      ...
      return reply
        .code(500)
        .send({ status: "error", text: "Error al guardar SSD" });
    }
  });
}
